...
async function sendSMS(recipients, message) {
  ...
  const normalized = to
    ...
    // Keep only valid Zambian mobile numbers: +260 followed by 9 digits starting with 9 or 7
    .filter((num) => /^\+260[97]\d{8}$/.test(num));

  if (normalized.length === 0) {
    return { success: false, error: "No valid phone numbers after validation" };
  }
